...
try:
    ...
    st.info("Place btc_1s.csv.gz / eth_1s.csv.gz in `data/binance_spot/`")
    st.stop()
...
for t in range(max(wl, cb//4), len(r), step):
...
if trades_df.empty:
    st.warning("⚠️ No trades generated. On real data you should get more setups. "
               "Try relaxing parameters (lower persist_k, lower quantile thresholds).")
    st.metric("Setups detected (state machine)", len(setups))
    st.stop()
