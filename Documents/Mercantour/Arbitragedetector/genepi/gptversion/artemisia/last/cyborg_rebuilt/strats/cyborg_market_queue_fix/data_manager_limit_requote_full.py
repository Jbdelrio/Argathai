...
class Coin5MinState(StrategyRuntime):
    ...
    target_sum_yes_no: float = 0.95
    no_new_entry_last_sec: int = 30
    max_entry_time_left_sec: int = 180
    signals: Dict[str, Dict] = field(default_factory=dict)
    pending_leg1_orders: Dict[str, Dict] = field(default_factory=dict)
    pending_hedge_orders: Dict[str, Dict] = field(default_factory=dict)
...
class LiveState:
    ...
    def reset_session(self, capital: Optional[float] = None, bm_alloc: Optional[float] = None, c5_alloc: Optional[float] = None) -> Tuple[bool, str]:
        with self._lock:
            ...
            self.coin5min.pending_leg1_orders = {}
            self.coin5min.pending_hedge_orders = {}
            self.coin_market_history = defaultdict(lambda: deque(maxlen=300))
    ...
    def update_coin5min_params(
        ...
        target_sum_yes_no: Optional[float] = None,
        no_new_entry_last_sec: Optional[int] = None,
        max_entry_time_left_sec: Optional[int] = None,
    ):
        with self._lock:
            ...
            if spot_source is not None and str(spot_source) != self.coin5min.spot_source:
                ...
                restart_warmup = True
                changed = True
            if target_sum_yes_no is not None and float(target_sum_yes_no) != self.coin5min.target_sum_yes_no:
                self.coin5min.target_sum_yes_no = max(0.80, min(0.999, float(target_sum_yes_no)))
                changed = True
            if no_new_entry_last_sec is not None and int(no_new_entry_last_sec) != self.coin5min.no_new_entry_last_sec:
                self.coin5min.no_new_entry_last_sec = max(5, int(no_new_entry_last_sec))
                changed = True
            if max_entry_time_left_sec is not None and int(max_entry_time_left_sec) != self.coin5min.max_entry_time_left_sec:
                self.coin5min.max_entry_time_left_sec = max(31, int(max_entry_time_left_sec))
                changed = True
    ...
    def _stop_one_locked(self, strategy: str):
        ...
        if strategy == "betmiss":
            ...
            s.partial_legs = {}
            s.pending_leg1_orders = {}
            s.pending_hedge_orders = {}
...
def _coin_first_leg_plan(signal: Dict, time_left_sec: Optional[float], c5: Coin5MinState) -> Optional[Dict]:
    if not c5.legging_enabled:
        return None
    if time_left_sec is None or time_left_sec <= c5.no_new_entry_last_sec:
        return None
    if time_left_sec > c5.max_entry_time_left_sec:
        return None

    yes_px = float(signal.get("market_yes_price", 1.0))
    no_px = float(signal.get("market_no_price", 1.0))
    target_sum = min(float(c5.max_sum_yes_no), float(c5.target_sum_yes_no))
    ...
    hedge_trigger = max(0.0, target_sum - first_price)
    # WAIT mode tries to improve the first leg price a bit rather than hitting the displayed ask immediately.
    score = float(signal.get('opportunity_score', 0.0) or 0.0)
    if score >= 0.70:
        first_limit = first_price
    else:
        first_limit = max(0.05, first_price - 0.02)

    return {
        'side': side,
        'first_price': first_price,
        'first_limit': round(first_limit, 4),
        'hedge_side': 'no' if side == 'yes' else 'yes',
        'hedge_price_now': hedge_price,
        'hedge_trigger': round(hedge_trigger, 4),
        'target_sum_yes_no': round(target_sum, 4),
    }
...
def _score_from_sum(sum_yes_no: Optional[float], max_sum_yes_no: float) -> float:
    if sum_yes_no is None:
        return 0.0
    x = float(sum_yes_no)
    hard_cap = float(max_sum_yes_no)
    if x <= hard_cap:
        return 1.0
    bad = 1.03
    if x >= bad:
        return 0.0
    return _clamp01((bad - x) / (bad - hard_cap))


def _score_from_trend(signal: Dict) -> float:
    trend_bps = abs(float(signal.get('trend_bps', 0.0) or 0.0))
    trend_dir = str(signal.get('trend_dir', 'flat') or 'flat')
    direction = str(signal.get('direction', 'flat') or 'flat')
    leg1_bias = str(signal.get('leg1_bias', direction) or direction)
    raw = min(trend_bps / 15.0, 1.0)
    aligned = ((leg1_bias == 'above' and direction == 'above') or (leg1_bias == 'below' and direction == 'below'))
    if trend_dir == 'flat':
        return 0.5 * raw
    score = 0.65 * raw + (0.35 if aligned else 0.10)
    return _clamp01(score)


def _score_from_spread(yes_bid, yes_ask, no_bid, no_ask) -> float:
    legs = []
    if yes_bid is not None and yes_ask is not None and float(yes_ask) > 0:
        legs.append(max(0.0, float(yes_ask) - float(yes_bid)))
    if no_bid is not None and no_ask is not None and float(no_ask) > 0:
        legs.append(max(0.0, float(no_ask) - float(no_bid)))
    if not legs:
        return 0.4
    avg_spread = float(np.mean(legs))
    if avg_spread <= 0.01:
        return 1.0
    if avg_spread >= 0.06:
        return 0.0
    return _clamp01((0.06 - avg_spread) / 0.05)


def _score_from_time_left(time_left_sec: Optional[float], no_new_entry_last_sec: int = 30) -> float:
    if time_left_sec is None:
        return 0.0
    t = float(time_left_sec)
    if t <= no_new_entry_last_sec:
        return 0.0
    if t >= 240:
        return 1.0
    if t >= 120:
    ...
    if t >= 60:
        return 0.60
    return _clamp01((t - no_new_entry_last_sec) / (60.0 - no_new_entry_last_sec))


def _compute_opportunity_score(signal: Dict, yes_bid, yes_ask, no_bid, no_ask, time_left_sec: Optional[float], max_sum_yes_no: float, no_new_entry_last_sec: int = 30) -> Dict:
    sum_score = _score_from_sum(signal.get('sum_yes_no'), max_sum_yes_no)
    trend_score = _score_from_trend(signal)
    spread_score = _score_from_spread(yes_bid, yes_ask, no_bid, no_ask)
    time_score = _score_from_time_left(time_left_sec, no_new_entry_last_sec=no_new_entry_last_sec)
    final_score = 0.45 * sum_score + 0.20 * trend_score + 0.15 * spread_score + 0.20 * time_score
    final_score = round(float(final_score), 4)
    if time_left_sec is not None and float(time_left_sec) <= float(no_new_entry_last_sec):
        decision = 'SKIP'
        reason = 'last_30s_block'
    elif signal.get('sum_yes_no') is not None and float(signal['sum_yes_no']) < 1.0 and final_score >= 0.70:
        decision = 'TRADE'
        reason = 'score_high_and_sum_lt_1'
    elif final_score >= 0.50:
        decision = 'WAIT'
        reason = 'watch_for_leg2_or_better_price'
    else:
        decision = 'SKIP'
    ...
    for asset in assets:
        ...
        if signal:
            ...
            opp = _compute_opportunity_score(
                ...
                max_sum_yes_no=min(float(c5.max_sum_yes_no), float(c5.target_sum_yes_no)),
                no_new_entry_last_sec=c5.no_new_entry_last_sec,
            )
            signal.update(opp)
            with ST._lock:
                ...
                c5.market_meta[asset].update({
                    ...
                    "sum_score": signal.get("sum_score"),
                    "trend_score_component": signal.get("trend_score_component"),
                    "spread_score": signal.get("spread_score"),
                    "time_score": signal.get("time_score"),
        ...
        if signal and not ST.kill_switch and total_budget >= 1.0 and window_running:
            ...
            opp_score = float(signal.get("opportunity_score", 0.0) or 0.0)
            entry_window_ok = (time_left is not None and c5.no_new_entry_last_sec < time_left <= c5.max_entry_time_left_sec)

            with ST._lock:
                pending_leg1 = c5.pending_leg1_orders.get(asset)
                pending_hedge = c5.pending_hedge_orders.get(asset)

            # direct simultaneous fill only if exceptional pricing
            if (
                decision == "TRADE"
                ...
                and total_cost
                and total_cost < 1.0
                and signal.get("sum_yes_no", 9) <= min(c5.max_sum_yes_no, c5.target_sum_yes_no)
                and entry_window_ok
            ):
                yes_budget = total_budget * (yes_buy / total_cost)
                no_budget = total_budget * (no_buy / total_cost)
                yes_order = ST.paper.place_order(strategy="coin5min", platform="polymarket", market_id=mkt["market_id"], question=mkt["question"], side="yes", market_price=yes_buy, size=yes_budget, asset=asset, direction="above", edge=signal["edge"])
                no_order = ST.paper.place_order(strategy="coin5min", platform="polymarket", market_id=mkt["market_id"], question=mkt["question"], side="no", market_price=no_buy, size=no_budget, asset=asset, direction="below", edge=signal["edge"])
                if yes_order and no_order:
                    pair_id = f"C5ARB-{int(time.time())}-{asset}"
                    row = _record_trade_row(pair_id, "coin5min", asset, "yes+no", signal["sum_yes_no"], total_budget, "OPEN", pnl=0.0, edge=signal["edge"], note=f"SIMUL YES {yes_buy:.4f} + NO {no_buy:.4f} | score={opp_score:.3f} | ref={ref_price:.2f}")
                    with ST._lock:
                        ST.recent_trades.insert(0, row)
                        c5.open_trade_ids[asset] = pair_id
                        c5.open_trade_meta[asset] = {"market_id": mkt["market_id"], "yes_order_id": yes_order.id, "no_order_id": no_order.id, "reference_price": ref_price, "entry_spot": spot["price"], "entry_ts": time.time(), "expiry_ts": float(mkt.get("end_ts") or (time.time() + mkt.get("interval_min", interval_min) * 60)), "market_question": mkt["question"], "mode": "simultaneous", "opportunity_score": opp_score}
                        c5.pending_leg1_orders.pop(asset, None)
                        c5.pending_hedge_orders.pop(asset, None)
                        open_exists = True
                ...
                # keep/update paper hedge limit order
                with ST._lock:
                    c5.pending_hedge_orders[asset] = {
                        "side": hedge_side,
                        "limit_price": float(partial_leg["hedge_trigger"]),
                        "current_ask": float(hedge_price_now),
                        "best_locked_sum": round(float(locked_cost), 4),
                ...
                if hedge_price_now <= partial_leg["hedge_trigger"] and locked_cost < 1.0:
                    ...
                    hedge_order = ST.paper.place_order(strategy="coin5min", platform="polymarket", market_id=mkt["market_id"], question=mkt["question"], side=hedge_side, market_price=hedge_price_now, size=hedge_budget, asset=asset, direction="above" if hedge_side == "yes" else "below", edge=locked_edge)
                    if hedge_order:
                        ...
                        _sync_trade_row(partial_leg["pair_id"], 0.0, "OPEN", note=(f"LEGGED {partial_leg['first_side'].upper()} {partial_leg['first_price']:.4f} -> {hedge_side.upper()} {hedge_price_now:.4f} | locked {locked_cost:.4f} | edge {locked_edge*100:.2f}% | score={opp_score:.3f}"))
                        with ST._lock:
                            c5.partial_legs.pop(asset, None)
                            c5.pending_hedge_orders.pop(asset, None)
                            c5.open_trade_ids[asset] = partial_leg["pair_id"]
                            c5.open_trade_meta[asset] = {"market_id": mkt["market_id"], "yes_order_id": yes_order_id, "no_order_id": no_order_id, "reference_price": partial_leg["reference_price"], "entry_spot": partial_leg["entry_spot"], "entry_ts": partial_leg["entry_ts"], "expiry_ts": partial_leg["expiry_ts"], "market_question": mkt["question"], "mode": "legged", "opportunity_score": opp_score}
                            open_exists = True
                            partial_leg = None
                elif pending_hedge and hedge_price_now > partial_leg['hedge_trigger']:
                    with ST._lock:
                        c5.pending_hedge_orders[asset]['status'] = 'working'
            elif not partial_leg and not open_exists and decision in ("TRADE", "WAIT") and entry_window_ok:
                plan = _coin_first_leg_plan(signal, time_left, c5)
                if plan:
                    current_leg_price = plan['first_price']
                    planned_limit = plan['first_limit']
                    # working limit order for leg1
                    if pending_leg1 is None or pending_leg1.get('side') != plan['side']:
                        with ST._lock:
                            c5.pending_leg1_orders[asset] = {
                                'side': plan['side'],
                                'limit_price': planned_limit,
                                'current_ask': current_leg_price,
                                'decision': decision,
                                'updated_at': time.time(),
                            }
                        pending_leg1 = c5.pending_leg1_orders.get(asset)
                    else:
                        # requote more aggressively only if market improves or signal upgrades to TRADE
                        new_limit = pending_leg1.get('limit_price', planned_limit)
                        if decision == 'TRADE':
                            new_limit = max(float(new_limit), float(current_leg_price))
                        else:
                            new_limit = min(float(new_limit), float(planned_limit))
                        with ST._lock:
                            c5.pending_leg1_orders[asset].update({'limit_price': round(float(new_limit),4), 'current_ask': current_leg_price, 'decision': decision, 'updated_at': time.time()})
                        pending_leg1 = c5.pending_leg1_orders.get(asset)

                    leg1_limit = float(pending_leg1.get('limit_price', planned_limit))
                    if current_leg_price <= leg1_limit:
                        first_budget = total_budget * 0.5
                        first_order = ST.paper.place_order(strategy="coin5min", platform="polymarket", market_id=mkt["market_id"], question=mkt["question"], side=plan["side"], market_price=current_leg_price, size=first_budget, asset=asset, direction=signal.get("direction"), edge=signal.get("edge"))
                        if first_order:
                            pair_id = f"C5LEG-{int(time.time())}-{asset}"
                            row = _record_trade_row(pair_id, "coin5min", asset, f"{plan['side']}_leg", current_leg_price, first_budget, "LEG1_OPEN", pnl=0.0, edge=signal.get("edge"), note=(f"LEG1 {plan['side'].upper()} {current_leg_price:.4f} | limit {leg1_limit:.4f} | wait {plan['hedge_side'].upper()} <= {plan['hedge_trigger']:.4f} | score={opp_score:.3f} | decision={decision}"))
                            with ST._lock:
                                ST.recent_trades.insert(0, row)
                                c5.pending_leg1_orders.pop(asset, None)
                                c5.pending_hedge_orders[asset] = {'side': plan['hedge_side'], 'limit_price': plan['hedge_trigger'], 'current_ask': plan['hedge_price_now'], 'updated_at': time.time(), 'status': 'working'}
                                c5.partial_legs[asset] = {"pair_id": pair_id, "market_id": mkt["market_id"], "market_question": mkt["question"], "first_side": plan["side"], "first_price": current_leg_price, "first_order_id": first_order.id, "hedge_side": plan["hedge_side"], "hedge_trigger": plan["hedge_trigger"], "remaining_budget": total_budget - first_budget, "entry_spot": spot["price"], "reference_price": ref_price, "entry_ts": time.time(), "expiry_ts": min(float(mkt.get("end_ts") or (time.time() + mkt.get("interval_min", interval_min) * 60)), time.time() + c5.max_leg_hold_sec), "opportunity_score": opp_score}
                                partial_leg = c5.partial_legs.get(asset)
        with ST._lock:
            ...
            if asset in c5.signals:
                c5.signals[asset].update({
                    ...
                    "opportunity_score": signal.get("opportunity_score") if signal else None,
                    "sum_score": signal.get("sum_score") if signal else None,
                    "trend_score_component": signal.get("trend_score_component") if signal else None,
                    "spread_score": signal.get("spread_score") if signal else None,
                    "time_score": signal.get("time_score") if signal else None,
                    "decision": signal.get("decision") if signal else None,
                    "decision_reason": signal.get("decision_reason") if signal else None,
                })
