"""
data_manager_real_live_switch_full.py – runtime state + live paper trading loops for CYBORG
...
from execution_engine import build_engine_from_env, EngineMode
...
def _compute_frequency_opportunity(
    ...
    opp_score = round(0.55 * sum_score + 0.20 * trend_score + 0.10 * spread_score + 0.15 * time_score, 4)
    ...
    if time_left_sec is not None and time_left_sec <= float(c5.no_new_entry_last_sec):
        ...
        decision = "SKIP"
        reason = "too_late"
    elif exec_sum is not None and exec_sum <= trade_cap:
        decision = "TRADE"
...
def _choose_leg1_frequency(signal: Dict, c5: Coin5MinState) -> Optional[Dict]:
    ...
    trade_cap = float(c5.target_sum_yes_no)
    target_edge = max(0.005, 1.0 - trade_cap)

    if c5.leg1_directional:
        if bias == "above" and yes_exec is not None and yes_exec <= c5.max_first_leg_price:
            return {
                "side": "yes",
                "first_price": float(yes_exec),
                "hedge_side": "no",
                "hedge_trigger": max(0.01, 1.0 - float(yes_exec) - target_edge),
            }
        if bias == "below" and no_exec is not None and no_exec <= c5.max_first_leg_price:
            return {
                "side": "no",
                "first_price": float(no_exec),
                "hedge_side": "yes",
                "hedge_trigger": max(0.01, 1.0 - float(no_exec) - target_edge),
            }

    # fallback cheapest executable side
    candidates = []
    if yes_exec is not None and yes_exec <= c5.max_first_leg_price:
        candidates.append(("yes", float(yes_exec)))
    if no_exec is not None and no_exec <= c5.max_first_leg_price:
        candidates.append(("no", float(no_exec)))
    if not candidates:
        return None
    ...
    for asset in assets:
        ...
        try:
            if mkt.get("token_yes"):
                ...
                bid_depth += sum(float(x.get("size", 0)) for x in _normalize_book_side(book_yes.get("bids", []), descending=True)[:8])
                ask_depth += sum(float(x.get("size", 0)) for x in _normalize_book_side(book_yes.get("asks", []), descending=False)[:8])
            if mkt.get("token_no"):
                ...
                bid_depth += sum(float(x.get("size", 0)) for x in _normalize_book_side(book_no.get("bids", []), descending=True)[:8])
                ask_depth += sum(float(x.get("size", 0)) for x in _normalize_book_side(book_no.get("asks", []), descending=False)[:8])
        except Exception:
            pass
...
def set_execution_mode(mode: str) -> Dict:
    ...
    with ST._lock:
        ST.execution_mode = mode
        ST.execution = build_engine_from_env(mode=mode)
        ST.execution_connected = False
        ST.execution_last_error = ""
    ST.alert("INFO", "EXEC", f"Execution mode set to {mode.upper()}")
    return {"ok": True, "mode": mode}


def connect_execution() -> Dict:
    with ST._lock:
        engine = ST.execution
    if engine is None:
    ...
    with ST._lock:
        ST.execution_connected = bool(result.get("ok"))
        ST.execution_last_error = str(result.get("error", "")) if not result.get("ok") else ""
    if result.get("ok"):
        ST.alert("INFO", "EXEC", f"{ST.execution_mode.upper()} execution connected")
    else:
        ST.alert("CRIT", "EXEC", f"Execution connect failed: {result.get('error')}")
    return result


def get_execution_alerts() -> List[Dict]:
    with ST._lock:
        engine = ST.execution
    return engine.get_alerts() if engine else []


def _mirror_execution_order(*, asset: str, token_id: Optional[str], side: str, price: float, size: float, tags: Optional[Dict] = None) -> Dict:
    """
    Sends the order to the selected execution engine.
    - paper mode: alert-capable mirror order book
    - live mode: real order routing through execution_engine.py

    IMPORTANT:
    The strategy state / pnl in this integration is still driven by PaperTradingEngine.
    Live execution routing is meant for phased rollout with tiny sizes first.
    """
    with ST._lock:
        engine = ST.execution
        connected = ST.execution_connected
        mode = ST.execution_mode

    if engine is None:
        return {"ok": False, "error": "execution_engine_missing"}
    if mode == "live" and not connected:
        return {"ok": False, "error": "live_engine_not_connected"}

    symbol = asset
    token = str(token_id or "")
    # Normalize to BUY for first integration; side is tracked in tags.
    order_side = "BUY"
    ...
    try:
        ...
        ST.alert("CRIT", "EXEC", f"Execution post failed: {exc}")
...
def start_workers():
    with ST._lock:
        if ST._started:
            return
        ST._started = True
        ST.execution = build_engine_from_env(mode=ST.execution_mode)
        ST.execution_connected = False
        ST.execution_last_error = ""
    init_db()
