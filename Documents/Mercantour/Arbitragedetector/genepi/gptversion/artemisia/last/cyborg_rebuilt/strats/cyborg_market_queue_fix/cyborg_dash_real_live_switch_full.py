...
from data_manager_real_live_switch_full import ST, get_pnl_curve, start_workers, set_execution_mode, connect_execution
    ...
    return html.Div(
        [
            html.Div(
                ...
                card([html.Div("EXECUTION ALERTS", style={"fontFamily": M, "fontSize": "11px", "color": T["dim"], "marginBottom": "8px"}), table_component("exec-alerts-table")]),
                        ], style={"flex": "1", "minWidth": "0"}
                    ),
                ], style={"display": "flex", "gap": "14px", "alignItems": "flex-start"}
            )
...
@app.callback(
    ...
    Output("exec-alerts-table", "data"),
    Output("exec-alerts-table", "columns"),
    Input("tick", "n_intervals"),
    Input("c5-edge", "value"),
    Input("c5-size", "value"),
    Input("c5-int", "value"),
...
def refresh_coin5min(_tick, edge, size, interval_min, assets, chart_asset, max_leg_hold, max_leg1, max_sum, max_trade_usd, directional_leg1, spot_source, exec_mode):
    s = ST.snap()
    c5 = s["coin5min"]
    status = html.Div([
    ...
    exec_alert_rows = []
    for a in (ex.get("alerts", []) or [])[:25]:
        exec_alert_rows.append({"time": a.get("ts"), "level": a.get("level"), "code": a.get("code"), "symbol": a.get("symbol"), "order_id": a.get("order_id"), "message": a.get("message"), "sound": a.get("sound_hint")})
    exec_alert_cols = [{"name": c.upper(), "id": c} for c in ["time","level","code","symbol","order_id","message","sound"]]
    ...
    return status, kpis, warmup_items, ranking_table(signals), market_live, coin_chart_spot(s, chart_asset), coin_chart_yesno(s, chart_asset), coin_chart_sum(s, chart_asset), coin_chart_score(s, chart_asset), signal_rows, signal_cols, trade_rows, trade_cols, exec_alert_rows, exec_alert_cols
