...
def coin_chart_sum(snapshot: Dict, asset: str) -> go.Figure:
    ...
    if hist:
        xs = list(range(len(hist)))
        sums = [r.get("sum_yes_no") for r in hist]
...
def coin_chart_score(snapshot: Dict, asset: str) -> go.Figure:
    ...
    if hist:
        ...
        score = float(sig.get("opportunity_score", 0.0) or 0.0)
                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            card([html.Div("🔥 Opportunity Ranking", style={"fontFamily": M, "fontSize": "11px", "color": T["dim"], "marginBottom": "8px"}), html.Div(id="c5-ranking-block")]),
...
@app.callback(Output("c5-conf-val", "children"), Input("c5-conf", "value"))
def c5_conf_value(v):
    return f"{v:.0f}%"
...
def refresh_coin5min(_tick, edge, size, interval_min, assets, chart_asset, max_leg_hold, max_leg1, max_sum, max_trade_usd, directional_leg1, spot_source):
    ...
    for asset in assets or []:
        ...
        signal_rows.append({
            ...
            "yes_ask": sig.get("yes_ask"),
            "no_ask": sig.get("no_ask"),
            "yes+no": sig.get("sum_yes_no"),
            "target_sum": c5.get("target_sum_yes_no"),
            "edge": f"{100*sig.get('edge',0):+.2f}%" if sig else "",
            ...
            "opp_score": sig.get("opportunity_score"),
    ...
    signal_cols = [{"name": c.upper(), "id": c} for c in ["asset", "warmup", "interval", "ref_px", "spot", "spot_src", "time_left_s", "yes_mark", "no_mark", "yes_ask", "no_ask", "yes+no", "target_sum", "edge", "state", "leg_state", "hedge_at", "budget_pct", "budget_cap_usd", "budget_effective_usd", "trend_bias_used", "opp_score", "sum_score", "trend_score", "spread_score", "time_score", "decision"]]
    ...
    return status, kpis, warmup_items, ranking_table(signals), coin_chart_spot(s, chart_asset), coin_chart_yesno(s, chart_asset), coin_chart_sum(s, chart_asset), coin_chart_score(s, chart_asset), signal_rows, signal_cols, trade_rows, trade_cols
